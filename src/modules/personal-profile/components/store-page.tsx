...
export function StoreListPage({ onBack }: StoreListPageProps) {
  ...
  const handleSelectStore = () => {
    ...
    // Add logic to save selected store
    onBack()
  }
  ...
  return (
    <div className="inset-0 bg-gray-100 flex flex-col z-50">
      ...
      <div className="bg-[#658C58] text-white p-4 flex items-center gap-4">
        ...
        <h1 className="text-xl font-bold flex-1 text-center mr-10">DANH SÁCH CỬA HÀNG</h1>
      </div>
      ...
      <div className="bg-blue-100 py-3 px-4 text-center">
        <span className="text-blue-600 font-bold text-lg">TỔNG ({filteredStores.length})</span>
      </div>
              ...
              <div className="flex-shrink-0">
                <div
                  className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${
                    selectedStore === store.id ? "border-blue-600 bg-blue-600" : "border-gray-300 bg-white"
                  }`}
                >
                  {selectedStore === store.id && <div className="w-3 h-3 rounded-full bg-white" />}
                </div>
              </div>
