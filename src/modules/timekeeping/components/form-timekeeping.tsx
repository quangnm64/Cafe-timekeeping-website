  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="text-center mb-8">
        ...
        <p className="text-muted-foreground">Hệ thống quản lý thời gian làm việc</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card className="p-6">
          <div className="text-center">
            ...
            {checkInTime ? (
              <div className="mb-4">
                <p className="text-sm text-muted-foreground mb-1">Đã chấm công lúc:</p>
                <p className="text-2xl font-bold text-[#658C58]">{formatTime(checkInTime)}</p>
              </div>
            ) : (
        ...
        <Card className="p-6">
          <div className="text-center">
            ...
            {checkOutTime ? (
              <div className="mb-4">
                <p className="text-sm text-muted-foreground mb-1">Đã chấm công lúc:</p>
                <p className="text-2xl font-bold text-[#658C58]">{formatTime(checkOutTime)}</p>
              </div>
            ) : (
              <p className="text-muted-foreground mb-4">Chưa chấm công</p>
            )}
      ...
      <Card className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="p-4 rounded-lg bg-muted">
            ...
            <p className="text-xl font-bold text-[#658C58]">{checkInTime ? formatTime(checkInTime) : "--:--:--"}</p>
          </div>
          <div className="p-4 rounded-lg bg-muted">
            <p className="text-sm text-muted-foreground mb-1">Giờ Ra</p>
            <p className="text-xl font-bold text-[#658C58]">{checkOutTime ? formatTime(checkOutTime) : "--:--:--"}</p>
          </div>
          <div className="p-4 rounded-lg bg-muted">
            <p className="text-sm text-muted-foreground mb-1">Tổng Giờ Làm</p>
            <p className="text-xl font-bold text-[#658C58]">
              {checkInTime && checkOutTime
                ? `${Math.floor((checkOutTime.getTime() - checkInTime.getTime()) / 3600000)}h ${Math.floor(((checkOutTime.getTime() - checkInTime.getTime()) % 3600000) / 60000)}m`
                : "--h --m"}
            </p>
          </div>
        </div>
